<template>
	<md-layout md-gutter md-align="center"md-column md-flex-xlarge="60" md-flex-large="60" md-flex-medium="60" md-flex-small="100" md-flex-xsmall="100">
				...
				<md-table>
					...
					<md-table-body>
						...
						<md-table-row v-for="(exercise, exIndex) in item.exercises" :key="exIndex">
							...
							<md-table-cell>
								<md-button class="md-icon-button" id="editExerciseButton" @click="editExerciseOn(item.id, exercise)">
			            <md-icon>edit</md-icon>
			          </md-button>
			          <md-button class="md-icon-button" id="deleteExerciseButton" @click="deleteExerciseOn(item.id, exercise.id)">
			            <md-icon>delete_forever</md-icon>
			          </md-button>
							</md-table-cell>
						</md-table-row>
		...
		<NewProgrammDialog ref="newProgramm" @fetchProgramms="fetchData" />

		<!-- Change programm's name -->
		<ChangeProgrammNameDialog :programmId="current_programm_id" :programmName="name" ref="changeName" @fetchProgramms="fetchData" />

		<!-- Dialog for delete programm -->
		<DeleteProgrammDialog :programm="current_programm_id" ref="deleteProgramm" @fetchProgramms="fetchData" />

    <!-- Dialog for delete exercise -->
    <DeleteExerciseDialog ref="deleteExercise" :programm="current_programm_id" :exercise="current_exercise_id" @fetchProgramms="fetchData" />

    <!-- Edit exericse dialog -->
    <EditExerciseDialog ref="editExercise" :programmId="current_programm_id" :exerciseId="current_exercise_id" :category="category" @fetchProgramms="fetchData" />

    <PlanningDialog ref="PlanningDialog" :id="current_programm_id" />
